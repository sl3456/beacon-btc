...
学习用，不是投资建议。小资金按 20～50 美元一笔。

## 能做什么

- Coinbase BTC/USD 实时价和 K 线（红涨绿跌）
- 本地 5 分钟逻辑回归给出方向和相对稳定的挂单价
- 盘口和刚成交只改仓位，不改方向
- 未登录时不展示预测，后台仍模拟并训练（省讲解 token）
- 模拟成交后本地小步训练；Grok 纠正错单、防未来函数
...
默认 `VITE_AUTH_ENABLED=false`，不用登录就能看预测。若要「登录才显示挂单价」，自行接好认证并把该开关设为 `true`。
